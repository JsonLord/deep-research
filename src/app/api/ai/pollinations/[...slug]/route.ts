...
const API_PROXY_BASE_URL = process.env.POLLINATIONS_API_BASE_URL || POLLINATIONS_BASE_URL || "https://text.pollinations.ai/openai";
...
async function handler(req: NextRequest, { params }: { params: Promise<{ slug: string[] }> }) {
  try {
    ...
    console.error("Proxy error (pollinations):", error);
